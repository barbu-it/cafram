...
class Node(CaframNode):
    ...
    def __init__(self, *args, node_obj=None, node_attr="_node", node_conf=None, **kwargs):

        # if len(args) > 0:
        #     node_obj = node_obj or args[0] or self
        node_obj = node_obj or self

        self._node = NodeCtrl(node_obj=node_obj, node_attr=node_attr, node_conf=node_conf, *args, **kwargs)
